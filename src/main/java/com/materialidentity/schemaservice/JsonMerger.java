...
public class JsonMerger {
  ...
  public static ObjectNode deepMerge(
    JsonNode source,
    ObjectNode target,
    String delimiter
  ) {
  ...
  private static JsonNode mergeValues(
    JsonNode valueTarget,
    JsonNode valueSource,
    String delimiter
  ) {
    if (valueSource.isObject() && valueTarget.isObject()) {
      return deepMerge(
        (ObjectNode) valueSource,
        (ObjectNode) valueTarget,
        delimiter
      );
    }
    if (valueSource.isArray() && valueTarget.isArray()) {
      return mergeArrays(
        (ArrayNode) valueSource,
        (ArrayNode) valueTarget,
        delimiter
      );
    }
  ...
  private static ArrayNode mergeArrays(
    ArrayNode arr1,
    ArrayNode arr2,
    String delimiter
  ) {
    ...
    for (int i = 0; i < maxLength; i++) {
      ...
      result.add(
        (val1 != null && val2 != null)
          ? mergeValues(val2, val1, delimiter)
          : Optional.ofNullable(val1).orElse(val2)
      );
    }
    return result;
  }
    ...
    return mapper
      ...
      .textNode(
        Optional
          ...
          .map(d -> val2.asText() + d + val1.asText())
          .orElse(val1.asText())
      );
  }
}
