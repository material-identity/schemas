...
const SCHEMA_TYPES = {
  ...
  Forestry: ['v0.0.1'],
  ForestrySource: ['v0.0.1', 'v1.0.0', 'v1.1.0'],
  ForestryOutput: ['v1.0.0'],
  TKR: ['v0.0.4'],
  "E-CoC": ['v1.0.0'],
  Bluemint: ['v1.0.0'],
  Metals: ['v0.0.1', 'v0.1.0', 'v0.1.1'],
  HKM: ['v1.0.0'],
};
...
function createAjvInstance() {
  const ajv = new Ajv2019({
    ...
    allErrors: true,
    discriminator: true,
    // Avoid IEEE-754 false negatives for decimal schema multiples such as 32.1 / 0.000001.
    // A seventh fractional digit still differs from the nearest multiple by about 0.1.
...
function detectCertificateType(certificate) {
  ...
  if (certificate.RefSchemaUrl) {
    if (certificate.RefSchemaUrl.includes('forestry-output-schemas')) {
      return 'ForestryOutput';
    }
    if (certificate.RefSchemaUrl.includes('forestry-source-schemas')) {
  ...
  if (certificate.DigitalMaterialPassport) {
    ...
    // Check for ForestryOutput - has Products array with ListOfSpecies
    if (certificate.DigitalMaterialPassport.Products &&
        Array.isArray(certificate.DigitalMaterialPassport.Products) &&
        certificate.DigitalMaterialPassport.Products.some(p => p.ListOfSpecies)) {
      return 'ForestryOutput';
    }
