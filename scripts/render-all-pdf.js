...
async function findFixtureFiles() {
  ...
  try {
    const schemaTypes = await fs.readdir(fixturesDir);
    
    for (const schemaType of schemaTypes) {
      const schemaPath = path.join(fixturesDir, schemaType);
      const stat = await fs.stat(schemaPath);
      
      if (stat.isDirectory() && schemaType !== 'LICENSE' && schemaType !== 'E-CoC') {
        const versions = await fs.readdir(schemaPath);
        
        for (const version of versions) {
          const versionPath = path.join(schemaPath, version);
          ...
          if (versionStat.isDirectory()) {
            ...
            for (const file of files) {
              if (file.endsWith('.json') && 
                ...
                const outputPath = path.join('tmp', `${schemaType}_${version}_${baseName}.pdf`);
...
async function renderAllPdfs() {
  ...
  for (let i = 0; i < fixtures.length; i++) {
    ...
    try {
      ...
      const stats = await fs.stat(fixture.output);
